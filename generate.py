...
def create_site(address,accent_color1,accent_color2,accent_color3,accent_contrast1,accent_contrast2,accent_contrast3,contact,logo_url,title,text_buffer,movies,max_width=4000,slideshow_auto=False,slideshow_rate=5):
	...
	for i in range(start, end, step):
		...
		for d in chunk:
			...
			html_buffer += f'<a href="{d.imdb_url}" style="color:{accent_color2}"><strong>IMDb</strong></a>\n'
			if d.trailer_url:
				html_buffer += f'<a href="{d.trailer_url}" style="color:{accent_color3}"><strong>Trailer</strong></a>\n'
			html_buffer += '</div>\n'
			html_buffer += '</div>\n'
